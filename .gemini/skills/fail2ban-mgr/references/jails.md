...
| `nginx-wp-rest` | 3 | 600s | 86400s | UFW + Cloudflare |
...
## Filter Notes
- `nginx-honeypot`: Targets specific paths (.env, .git, wp-admin/install.php, phpinfo.php, vendor/phpunit, composer.json)
- `nginx-exploits`: Scoped to query strings only to avoid false positives on legitimate content
- `recidive`: Uses official Fail2Ban NOTICE pattern matching
