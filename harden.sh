...
# Create security-traps.conf (honeypot endpoints)
# Use MY_IP from environment for the wp-cron allow rule
cat > /etc/nginx/snippets/security-traps.conf <<EOF
# ==============================================================================
# SECURITY TRAPS - HONEYPOT & KILL SWITCHES
# Auto-generated by harden.sh - Do not edit manually
# ==============================================================================
# Include this in your server {} block:
...
# Scanner trap endpoints (honeypot)
...
# Tarpit scanners (slow them down)
...
# Add MY_IP to wp-cron allow rule if configured
if [[ -n "${MY_IP:-}" ]]; then
    cat >> /etc/nginx/snippets/security-traps.conf <<EOF
    allow ${MY_IP}; # Your home IP (from MY_IP in .env)
EOF
fi
...
echo "Created /etc/nginx/snippets/security-traps.conf"
